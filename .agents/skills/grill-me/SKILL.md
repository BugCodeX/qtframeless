---
name: grill-me
description: "Trigger: grill-me, loose idea, feature idea, tengo una idea, quiero agregar, entrevistarme, clarify intent before spec. Interview the user about a loose idea until it hardens into decisions."
license: MIT
metadata:
...
Use this skill when the user has a loose idea, feature direction, or design decision and needs to be interviewed before writing any spec, proposal, or code. Always invoke before `sdd-propose` when intent is vague. Prefer `grill-with-docs` instead when working inside a repo with an existing codebase.
...
Which skill you want depends on what is in front of you:
...
- **Too big for one session**: use SDD (`sdd-propose` → `sdd-spec` → `sdd-design` → `sdd-apply`). SDD is the multi-session planning flow for this project.
...
"One long form or three pages?" and "how should this interaction feel?" are **ungrillable**: they need something to react to. When you hit one, stop grilling. Build the throwaway version — a quick prototype — look at it, then come back and answer in one line.
...
No. The value of the session is the context you just built. Hand the same conversation straight to `sdd-propose`.
...
That portability is the whole difference from `grill-with-docs`, which runs the same interview but reads a codebase to align against and records what it learns as `CONTEXT.md` and ADRs. Both follow the same grilling approach; `grill-me` is the user-invoked front door that carries nothing with it.

If what you grilled does turn out to be software in this repo, hand the same conversation to `sdd-propose` and carry on into the SDD build flow: `sdd-propose` → `sdd-spec` → `sdd-design` → `sdd-apply`. That is the equivalent of the upstream `to-spec → to-tickets → implement` chain, adapted for this project.
